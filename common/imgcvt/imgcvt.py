...
def Image_convert(Source:Image.Image, in_pal:list, out_pal:list, gfxmode:gfxmodes=gfxmodes.C64MULTI, dither:DT.dithertype=DT.dithertype.BAYER8, threshold:int=4 , cmatch:int=None, bg_color=None):
    ...
    in_PaletteH = Palette.Palette(hd_in)    # Palette to dither/quantize against
    out_PaletteH = Palette.Palette(hd_out)  # Palette to display
    y_PaletteH = Palette.Palette(y_in)  # Luminance palette to dither/quantize against
    ...
    if Mode['attr']!=(0,0):
        ...
        if True in bestbg:
            ...
            en_img = np.zeros([height,width,3],dtype='uint8')
